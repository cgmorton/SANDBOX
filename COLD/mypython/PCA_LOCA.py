...
def get_PCA_data(var_name, rcp, years, data_dir):
    '''
...
if __name__ == '__main__':
    ...
    for model in LOCA_CMIP5_MODELS.keys():
        print('PROCESSSING MODEL ' + model)
        with open('PCA_output.txt','a') as out_f:
            out_f.write('PROCESSSING MODEL ' + model)
        for rcp in rcps:
            data_dir = '/media/DataSets/loca/' + model + '/' + rcp + '/'
            for var_name in ['tmin', 'tmax']:
                print('PROCESSSING VARIABLE ' + var_name)
                with open('PCA_output.txt','a') as out_f:
                    out_f.write('PROCESSSING VARIABLE ' + var_name)
                ...
                with open('PCA_output.txt','a') as out_f:
                    out_f.write('VARIANCE EXPLAINED: ' + str(pca.explained_variance_ratio_))
                ...
                for c_idx in range(num_comps):
                    ...
                    ts_data_comp = rotated_components[c_idx].dot(PCA_data.T)
                    std_ts_comp =  np.std(ts_data_comp)
                    ts_data_comp /= std_ts_comp
                    corr_coeffs = []
                    corr_p_values= []
                    for loc_idx  in range(PCA_data.shape[1]):
                        corr = correlate_ts(PCA_data[:,loc_idx], ts_data_comp)
                    ...
                    ts_data_corr = corr_coeffs.dot(PCA_data.T)
                    ...
                    with open(data_dir + var_name + '_' + rcp  + '_pca_component_' + str(c_idx+1) + '_ts.json', 'w') as outfile:
                        json.dump(hc_ts_data_comp, outfile)
                    with open(data_dir + var_name + '_' + rcp + '_pca_correlation_' + str(c_idx+1) + '_ts.json', 'w') as outfile:
                        json.dump(hc_ts_data_corr, outfile)
                    #print('Saving tif')
                    #Generate tif for component
                    out_file = data_dir + var_name + '_' + rcp + '_pca_component_' + str(c_idx + 1) + '.tif'
                    array_to_raster(lats, lons, rotated_components[c_idx] * std_ts_comp, out_file, nodata=-9999)
                    out_file = data_dir + var_name + '_' + rcp + '_pca_correlation_' + str(c_idx + 1) + '.tif'
